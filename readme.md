...
Nesse caso, o usuário deve tomar as medidas necessárias para que essa comunicação seja efetivada, entre essas medidas estão:  
a) configurar o firewall, tanto do SINAN NET servidor como no PC cliente, para possibilitar o uso da porta TCP 5445, que é a porta usada pelo PostgreSQL, o banco de dados relacional usado pelo SINAN NET. Geralmente isso é feito no firewall criando exceções de entrada e saída para essa porta específica;  
b) usar ferramentas como o comando PING, usado para solucionar problemas de conectividade entre os PCs;  
c) se utilizar de outros meios ou recursos necessários que o usuário ou o administrador de rede achar conveniente para estabelecer a comunicação esperada.  
